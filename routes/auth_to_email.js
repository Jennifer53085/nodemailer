...
router.get("/emailbySMTP", async (req, res) => {
  try {
    ...
    sendEmailbySMTP();
     // 返回 JSON 格式的回應
     res.status(200).json({ success: true, message: 'Email sent via SMTP.' });
  } catch (err) {
...
// 2.透過SMTP的方法(POST)，信件內容由前端設定
// router.post("/emailbySMTP", async (req, res) => {
//   try {
//     // 直接調用sendEmail來發送郵件
//     const postData = await req.body;
//     sendEmailbySMTP(postData);
